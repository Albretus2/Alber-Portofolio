export default function Footer() {
  return (
    <footer className="footer py-16 px-10 bg-primary text-dark border-y-[3px] border-dark font-semibold">
      <aside>
        <div className="playlist w-full mb-8 py-2 px-2 bg-secondary rounded-full flex flex-wrap items-center gap-3 shadowSpotify transition-all">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            x="0px"
            y="0px"
            width="50"
            height="50"
            viewBox="0 0 100 100">
            <circle cx="50" cy="50" r="35" fill="#60be92"></circle>
            <path
              fill="#1f212b"
              d="M50,86c-19.851,0-36-16.149-36-36s16.149-36,36-36s36,16.149,36,36S69.851,86,50,86z M50,16	c-18.748,0-34,15.252-34,34s15.252,34,34,34s34-15.252,34-34S68.748,16,50,16z"></path>
            <path d="M65.5,24.227c-0.086,0-0.174-0.022-0.254-0.069c-0.642-0.379-1.303-0.738-1.967-1.066c-0.248-0.123-0.349-0.422-0.227-0.67	s0.421-0.348,0.67-0.227c0.686,0.339,1.37,0.71,2.033,1.102c0.238,0.141,0.316,0.447,0.176,0.685	C65.837,24.139,65.67,24.227,65.5,24.227z"></path>
            <path d="M68.5,26.249c-0.106,0-0.212-0.034-0.303-0.103c-0.323-0.247-0.651-0.487-0.983-0.719c-0.226-0.159-0.281-0.471-0.123-0.697	c0.159-0.226,0.471-0.281,0.697-0.123c0.343,0.241,0.683,0.489,1.017,0.745c0.219,0.167,0.261,0.481,0.093,0.701	C68.799,26.182,68.65,26.249,68.5,26.249z"></path>
            <path
              fill="#1f212b"
              d="M50,81c-17.093,0-31-13.907-31-31s13.907-31,31-31c3.667,0,7.257,0.635,10.672,1.887	c0.259,0.095,0.393,0.382,0.297,0.641c-0.095,0.26-0.382,0.396-0.641,0.297C57.024,20.614,53.549,20,50,20	c-16.542,0-30,13.458-30,30s13.458,30,30,30s30-13.458,30-30c0-8.439-3.585-16.536-9.836-22.213	c-0.205-0.186-0.22-0.502-0.034-0.706c0.186-0.205,0.503-0.219,0.706-0.034C77.295,32.913,81,41.279,81,50	C81,67.093,67.093,81,50,81z"></path>
            <path
              fill="#fff"
              d="M72.167,46.852c-0.62,0-1.248-0.165-1.816-0.51c-17.023-10.355-39.833-4.366-40.061-4.304 c-1.866,0.501-3.787-0.598-4.293-2.461c-0.506-1.864,0.592-3.785,2.455-4.293c1.049-0.286,25.901-6.866,45.538,5.078 c1.651,1.004,2.176,3.158,1.171,4.809C74.501,46.253,73.349,46.852,72.167,46.852z"></path>
            <path
              fill="#1f212b"
              d="M72.167,47.352c-0.731,0-1.449-0.202-2.076-0.583c-16.896-10.278-39.444-4.31-39.669-4.249 c-2.125,0.571-4.33-0.688-4.907-2.813c-0.577-2.127,0.682-4.328,2.806-4.907c1.057-0.289,26.112-6.92,45.929,5.133 c0.913,0.555,1.555,1.433,1.808,2.471c0.253,1.039,0.086,2.113-0.47,3.025C74.855,46.633,73.576,47.352,72.167,47.352z M45.828,39.738c7.599,0,16.79,1.314,24.783,6.176c0.471,0.286,1.009,0.438,1.557,0.438c1.058,0,2.017-0.539,2.566-1.441 c0.416-0.685,0.541-1.491,0.352-2.269c-0.189-0.779-0.671-1.437-1.355-1.853c-19.453-11.832-44.106-5.305-45.146-5.022 c-1.593,0.434-2.537,2.085-2.104,3.68c0.433,1.592,2.085,2.539,3.681,2.109C30.282,41.521,36.975,39.738,45.828,39.738z"></path>
            <path
              fill="#fff"
              d="M68.146,57.267c-0.531,0-1.069-0.141-1.556-0.437c-14.728-8.958-34.318-3.763-34.514-3.71 c-1.599,0.437-3.248-0.507-3.684-2.104c-0.436-1.599,0.507-3.248,2.105-3.684c0.902-0.246,22.304-5.912,39.21,4.372 c1.416,0.861,1.865,2.707,1.005,4.122C70.147,56.754,69.159,57.267,68.146,57.267z"></path>
            <path
              fill="#1f212b"
              d="M68.146,57.767c-0.64,0-1.268-0.176-1.815-0.51c-14.467-8.798-33.929-3.707-34.125-3.654 c-0.9,0.247-1.843,0.126-2.656-0.338c-0.812-0.464-1.394-1.216-1.64-2.117c-0.507-1.862,0.594-3.791,2.456-4.298 c0.912-0.249,22.513-5.968,39.602,4.427c1.648,1.003,2.175,3.16,1.172,4.809C70.499,57.138,69.38,57.767,68.146,57.767z M45.448,51.097c6.579,0,14.532,1.127,21.403,5.307c0.392,0.237,0.839,0.363,1.296,0.363c0.882,0,1.682-0.449,2.14-1.201 c0.716-1.178,0.34-2.719-0.838-3.436c-16.724-10.171-37.925-4.561-38.819-4.316c-1.33,0.363-2.116,1.74-1.754,3.07 c0.176,0.644,0.592,1.181,1.171,1.512c0.581,0.332,1.256,0.418,1.898,0.241C32.053,52.608,37.828,51.097,45.448,51.097z"></path>
            <g>
              <path
                fill="#fff"
                d="M63.743,67.011c-0.443,0-0.892-0.117-1.297-0.364c-12.812-7.793-29.852-3.276-30.022-3.229 c-1.332,0.361-2.707-0.423-3.07-1.754c-0.363-1.332,0.422-2.707,1.754-3.07c0.781-0.212,19.307-5.117,33.936,3.782 c1.18,0.718,1.555,2.255,0.837,3.435C65.41,66.584,64.587,67.011,63.743,67.011z"></path>
              <path
                fill="#1f212b"
                d="M63.743,67.511c-0.55,0-1.088-0.151-1.558-0.438c-12.626-7.682-29.461-3.221-29.629-3.174 c-1.598,0.434-3.25-0.512-3.685-2.104c-0.435-1.597,0.509-3.249,2.105-3.685c0.792-0.215,19.548-5.153,34.327,3.838 c0.685,0.416,1.166,1.074,1.356,1.853c0.189,0.778,0.064,1.584-0.353,2.269C65.76,66.972,64.801,67.511,63.743,67.511z M44.06,61.569c5.714,0,12.628,0.989,18.646,4.65c0.916,0.558,2.193,0.243,2.747-0.67c0.278-0.456,0.361-0.993,0.235-1.513 c-0.127-0.519-0.448-0.957-0.904-1.235c-14.415-8.768-32.77-3.937-33.545-3.727c-1.063,0.29-1.693,1.393-1.403,2.456 c0.29,1.063,1.391,1.696,2.457,1.403C32.385,62.909,37.41,61.569,44.06,61.569z"></path>
            </g>
          </svg>
          <span>
            <p className="">one repeat</p>
            <a
              href="https://open.spotify.com/intl-id/album/5yaSbW4qEkpvqOiFfnujyN"
              target="_blank">
              {" "}
              ･－･･ ･ － ･･ － －－･ －－－
            </a>
          </span>
        </div>
        <p>@2024 A. Ichvanius Giharta 🔥 build with react + daisyUi</p>
      </aside>
      <nav>
        <h6 className="font-semibold opacity-50">Quick Action</h6>
        <a className="link link-hover italic">Download CV</a>
        <a href="mailto:aichvan22@gmail.com" className="link link-hover italic">
          Email me
        </a>
        <a
          href="https://www.linkedin.com/in/a-ichvanius-giharta-5011a0308/"
          className="link link-hover italic">
          Linkedin
        </a>
        <a href="https://github.com/Albretus2" className="link link-hover">
          Github
        </a>
      </nav>
      <nav>
        <h6 className="font-semibold opacity-50">Trakterr me a Coffee</h6>
        <a className="link link-hover italic">Saweria</a>
      </nav>
    </footer>
  );
}
